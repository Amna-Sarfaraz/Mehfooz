...
export default function MutualFundsPage() {
  ...
  const lesson = mutualFundLessons[current];
  const total = mutualFundLessons.length;
  ...
  const updateProgress = async (lessonIndex) => {
    ...
    await supabase.from('user_progress').upsert(
      { user_id: user.id, module_slug: 'mutual-funds', progress, completed, updated_at: new Date().toISOString() },
      { onConflict: 'user_id,module_slug' }
    );
  };

  const goTo = (idx) => { setCurrent(idx); updateProgress(idx); };
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f1eb', fontFamily: 'Georgia, serif', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: '#1a3a2a', padding: '2rem 2rem 0' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto' }}>
          <Link href="/learn" style={{ fontSize: '13px', color: '#7aaa8a', textDecoration: 'none', fontFamily: 'Arial, sans-serif', display: 'inline-flex', alignItems: 'center', gap: '6px', marginBottom: '1rem' }}>← All modules</Link>
          <p style={{ fontSize: '11px', fontWeight: '700', letterSpacing: '0.12em', color: '#d4a853', marginBottom: '0.5rem', fontFamily: 'Arial, sans-serif' }}>MUTUAL FUNDS</p>
          <h1 style={{ fontSize: '2rem', fontWeight: '400', color: '#f4f1eb', margin: '0 0 0.4rem', lineHeight: 1.2 }}>{lesson.title}</h1>
          <p style={{ fontSize: '13px', color: '#7aaa8a', fontFamily: 'Arial, sans-serif', margin: '0 0 1.5rem' }}>Lesson {current + 1} of {total} · {lesson.subtitle}</p>
          <div style={{ height: '3px', background: 'rgba(255,255,255,0.1)', borderRadius: '99px', marginBottom: '1.5rem' }}>
            <div style={{ height: '3px', background: '#d4a853', borderRadius: '99px', width: `${Math.round(((current + 1) / total) * 100)}%`, transition: 'width 0.4s ease' }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ borderTop: '1px solid #e2ded6', background: '#fff', padding: '1rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: '6px' }}>
          {mutualFundLessons.map((_, i) => (
            <div key={i} onClick={() => goTo(i)} style={{ width: i === current ? '24px' : '8px', height: '8px', borderRadius: '4px', background: i === current ? '#1a3a2a' : '#d1d5db', cursor: 'pointer', transition: 'all 0.3s' }} />
          ))}
        </div>
        {current === total - 1 ? (
          ...
          <button onClick={() => goTo(Math.min(total - 1, current + 1))}
      ...
      <div style={{ background: '#f4f1eb', borderTop: '1px solid #e2ded6', padding: '0.75rem 2rem' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '10px', border: '1px solid #e2ded6', borderRadius: '24px', padding: '10px 16px', background: '#fff' }}>
          ...
          <input id="mahfooz-chat-input" data-module="mutual-funds" type="text" placeholder="Ask anything about this topic..."
            style={{ flex: 1, border: 'none', background: 'transparent', fontSize: '14px', color: '#333', outline: 'none', fontFamily: 'Arial, sans-serif' }} />
          <button id="mahfooz-chat-send"
